...
#pragma omp parallel for
    for (int i = 0; i < lengths_boxes[0]; i++) {
        boxes_float[indexes_boxes[0] + i] = result[11][i];
    }
#pragma omp parallel for
    for (int i = 0; i < lengths_boxes[1]; i++) {
        boxes_float[indexes_boxes[1] + i] = result[9][i];
    }
#pragma omp parallel for
    for (int i = 0; i < lengths_boxes[2]; i++) {
        boxes_float[indexes_boxes[2] + i] = result[7][i];
    }
#pragma omp parallel for
    for (int i = 0; i < lengths_boxes[3]; i++) {
        boxes_float[indexes_boxes[3] + i] = result[5][i];
    }
#pragma omp parallel for
    for (int i = 0; i < lengths_boxes[4]; i++) {
        boxes_float[indexes_boxes[4] + i] = result[3][i];
    }
#pragma omp parallel for
    for (int i = 0; i < lengths_boxes[5]; i++) {
        boxes_float[indexes_boxes[5] + i] = result[1][i];
    }

#pragma omp parallel for
    for (int i = 0; i < lengths_clses[0]; i++) {
        clses_float[indexes_clses[0] + i] = result[10][i];
    }
#pragma omp parallel for
    for (int i = 0; i < lengths_clses[1]; i++) {
        clses_float[indexes_clses[1] + i] = result[8][i];
    }
#pragma omp parallel for
    for (int i = 0; i < lengths_clses[2]; i++) {
        clses_float[indexes_clses[2] + i] = result[6][i];
    }
#pragma omp parallel for
    for (int i = 0; i < lengths_clses[3]; i++) {
        clses_float[indexes_clses[3] + i] = result[4][i];
    }
#pragma omp parallel for
    for (int i = 0; i < lengths_clses[4]; i++) {
        clses_float[indexes_clses[4] + i] = result[2][i];
    }
#pragma omp parallel for
    for (int i = 0; i < lengths_clses[5]; i++) {
        clses_float[indexes_clses[5] + i] = result[0][i];
    }
}
...
int SSDPostProcessor::filter_results(float* init_scores, float* init_boxes,
    ...
    //TODO(yong): 문법상의 문제로 num_threads 변수를 임시로 사용한다.
